...
# import pygame  # Disabled due to numpy compatibility issues
import threading
import time

class AlertSystem:
    def __init__(self, config):
        # pygame.mixer.init()  # Disabled due to numpy compatibility issues
        self.config = config
        ...
        def _play_audio():
            try:
                if alert_type in self.alerts:
                    ...
                    # Play audio (pygame disabled, using print instead)
                    # pygame.mixer.music.load(temp_path)
                    # pygame.mixer.music.play()
                    print(f"🔊 ALERT: {self.alerts[alert_type]}")
                    
                    # Wait simulation (pygame disabled)
